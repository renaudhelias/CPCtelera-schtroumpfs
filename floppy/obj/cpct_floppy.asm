...
;--------------------------------------------------------
	.area _CODE
;src/cpct_floppy.c:2: void StoreDriveLetter(void)
;	---------------------------------
; Function StoreDriveLetter
; ---------------------------------
_StoreDriveLetter::
;src/cpct_floppy.c:21: __endasm;
;;------------------------------------------------------------------------
...
;ld	hl,(#0xbD38)
	...
	ld	(_firmJump+1),a
	ret
;src/cpct_floppy.c:27: void InitializeAmsdos(void) __naked
;	---------------------------------
; Function InitializeAmsdos
; ---------------------------------
_InitializeAmsdos::
;src/cpct_floppy.c:99: __endasm;
...
;and	high kernal jumpblock (&b800-&bae4)
	      _firmJump:
	call #0x08bd ;initialise firmware jumpblock entries
;call	#0xBD37
;(&bb00-	...)
...
;;	This will restore the drive number to the drive the loader was run from
	...
	ld	(hl),a
	ret
;src/cpct_floppy.c:103: void cpct_loadBinaryFile(const char *fileName, void *loadAddr) {
;	---------------------------------
; Function cpct_loadBinaryFile
; ---------------------------------
_cpct_loadBinaryFile::
;src/cpct_floppy.c:157: __endasm;
	ld	hl, #2
	add	hl, sp
	ld	e, (hl) ;; filename
	inc	hl
